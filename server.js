...
async function generateKeyAuthLicense(productConfig) {
  try {
    ...
    const response = await require('axios').get(`https://keyauth.win/api/seller/?${params.toString()}`);
...
app.post('/webhook/payhip', async (req, res) => {
  ...
  try {
    ...
    if (!buyer_email) { console.error('❌ Missing email'); return res.status(400).json({ error: 'Missing email' }); }
    if (!product_key && !product_id) { console.error('❌ Missing product info'); return res.status(400).json({ error: 'Missing product info' }); }
    ...
    if (!productConfig) {
      for (const [key, config] of Object.entries(CONFIG.PRODUCTS)) {
        if (product_key === key || product_id === key) { productConfig = config; break; }
      }
    }
    if (!productConfig) {
      console.log('⚠️ Product not mapped, using default');
    ...
    if (!licenseKey) { console.error('❌ Failed to generate license key'); return res.status(500).json({ error: 'Failed to generate license key' }); }

    const emailSent = await sendLicenseEmail(buyer_email, buyer_email.split('@')[0], licenseKey, product_name || productConfig.keyauthApp);
    if (!emailSent) { console.error('❌ Failed to send email'); }

    console.log('✅ Webhook processed successfully!\n');
    res.status(200).json({ success: true, message: 'License key generated and email sent', key: licenseKey });
  } catch (error) { console.error('❌ Webhook error:', error); res.status(500).json({ error: 'Internal server error' }); }
});

app.get('/test', async (req, res) => {
  res.json({ status: 'Server is running!', time: new Date().toISOString(),
    config: { emailConfigured: !!CONFIG.EMAIL_USER, keyauthConfigured: !!CONFIG.KEYAUTH_SELLER_KEY }
  });
});

app.get('/test/generate', async (req, res) => {
  const key = await generateKeyAuthLicense({ keyauthApp: 'SJTweaks Premium Utility', expiry: 0, level: 1, mask: 'XXXXX-XXXXX-XXXXX-XXXXX' });
